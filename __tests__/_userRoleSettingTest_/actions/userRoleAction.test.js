...
describe('User role setting  actions', () => {
	...
	it('should update role to users ', async () => {
		...
		await store.dispatch(updateUserRole({ userId: 92, role: 'Manager' })).then(async () => {
	...
	it('should display error when role is not supported', async () => {
		...
		const store = mockStore({});
		await store.dispatch(updateUserRole({ userId: 3, role: 'admin123454' })).then(async () => {
			const result = store.getActions();
...
const allUsers=[{id:3,role:'manager'},{id:5,role:'requester'},{id:15,role:'admin'}]
