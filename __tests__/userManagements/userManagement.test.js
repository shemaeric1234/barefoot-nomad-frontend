...
describe('User management component tests', () => {
	...
	it('should handle getOptionLabel', () => {
		...
		component.find('WithStyles(ForwardRef(Autocomplete))').at(0).props().getOptionLabel(mockedEvent);
		component.find('WithStyles(ForwardRef(Autocomplete))').at(1).props().getOptionLabel(mockedEvent);
		component.find('WithStyles(ForwardRef(Autocomplete))').at(0).props().renderInput({});
		component.find('WithStyles(ForwardRef(Autocomplete))').at(1).props().renderInput({});
	});
	...
	it('should handle onChange attribute in autocomplete', () => {
		...
		expect(handleOnChangeSpy).toHaveBeenCalled();
		expect(handleOnChangeSpy).toHaveBeenCalledWith(1, event.target.value, mockedUser);
